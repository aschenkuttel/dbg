...
if (window.location.href.indexOf('screen=place&mode=scavenge_mass') < 0) {
    ...
    console.log("Relocating to mass scavenging page121dqwddqwqdwdqwadqwadqwdqwdqwdq");
}
...
// wait for page to load, then get category names
$(function () {
    console.log(
        $('script:contains("ScavengeMassScreen")').toArray()
    );

    categoryNames = JSON.parse("[" + $.find('script:contains("ScavengeMassScreen")')[0].innerHTML.match(/\{.*\:\{.*\:.*\}\}/g) + "]")[0];
});
...
function getData() {
    ...
    $.get(URLReq, function (data) {
        ...
        .done(function () {
                ...
                $.getAll(URLs,
                    (i, data) => {
                        var contents = $(data).find('script:contains("ScavengeMassScreen")').html().match(/\{"spear".*\:\{.*\:.*\}\}/g)

                        if (contents.length === 0) {
                            console.error("No villages on page " + i);
                            return;
                        }

                        thisPageData = contents[0];
